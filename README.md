...
Initially, I went for a full A* search from the start position to the goal state. As I quickly realized, my heuristics were not good enough and the solver did not terminate in a reasonable time. For my heuristics, I use the Manhattan distance of every number to its target position, along with an addition of two moves for all linear conflicts (two numbers in the right row but in the wrong order). 
